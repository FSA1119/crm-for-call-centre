...
const CRM_CONFIG = {
    ...
    // Color codes for status (RGB format)
    COLOR_CODES: {
      ...
      // Opportunity Colors
      'Fırsat İletildi': 'rgb(147, 112, 219)',     // Medium Purple
      'Bilgi Verildi': 'rgb(199, 171, 235)',        // Light Purple
    ...
    // Sheet header colors (RGB format)
    SHEET_HEADER_COLORS: {
      'Format Tablo': 'rgb(173, 216, 230)',
      'Randevularım': 'rgb(52, 168, 83)',
      'Fırsatlarım': 'rgb(251, 188, 5)',
      'Toplantılarım': 'rgb(234, 67, 53)'
    },
  ...
  function processAppointmentForm(formData) {
    ...
    try {
      ...
      if (activeSheet.getName() === 'Fırsatlarım') {
        try {
          console.log('🔄 Applying appointment color coding to Fırsatlarım');
          applyFirsatlarimAppointmentColorCoding(activeSheet, activeRange.getRow());
        } catch (error) {
          console.warn('⚠️ Failed to apply appointment color coding:', error.message);
        }
      }
  ...
  function applyFormatTableColorCoding(sheet, rowNumber, activity) {
    ...
    if (activity === 'Randevu Alındı' || activity === 'İleri Tarih Randevu') {
      ...
      console.log('🎨 Applied Randevu Alındı/İleri Tarih Randevu color:', color);
      ...
      console.log('🔍 DEBUG: Fırsat İletildi activity detected in Format Tablo row:', rowNumber);
      console.log('🔍 DEBUG: Color being applied:', color);
      console.log('🔍 DEBUG: CRM_CONFIG.COLOR_CODES[Fırsat İletildi]:', CRM_CONFIG.COLOR_CODES['Fırsat İletildi']);
  ...
  function addOpportunity(parameters = {}) {
    ...
    try {
      ...
      // Check if row already has opportunity status
      if (selectedRowData.Aktivite === 'Fırsat İletildi' || 
          selectedRowData.Aktivite === 'Bilgi Verildi' || 
          selectedRowData.Aktivite === 'Yeniden Aranacak') {
        console.log('Debug - Row already has opportunity status:', selectedRowData.Aktivite);
        // Allow re-adding opportunity if status is different
        console.log('Debug - Allowing re-adding opportunity with different status');
      }
      
      // Check if this row already exists in Fırsatlarım using Kod (more reliable)
      ...
      if (firsatlarimSheet) {
        ...
        console.log('Debug - selectedRowData.Kod:', selectedRowData.Kod);
        console.log('Debug - kodIndex:', kodIndex);
        console.log('Debug - Fırsatlarım data rows:', firsatlarimData.length);
        
        if (kodIndex !== -1 && selectedRowData.Kod && selectedRowData.Kod.toString().trim() !== '') {
          
          const selectedKod = selectedRowData.Kod.toString().trim();
          
          console.log('Debug - Checking for existing opportunity with Kod:', selectedKod);
          
          const existingOpportunity = firsatlarimData.slice(1).find(row => {
            const rowKod = row[kodIndex];
            const kodMatch = rowKod && rowKod.toString().trim() === selectedKod;
            
            console.log('Debug - Comparing rowKod:', rowKod, 'with selectedKod:', selectedKod, 'kodMatch:', kodMatch);
            
            return kodMatch;
          });
          
          if (existingOpportunity) {
            console.log('Debug - Found existing opportunity with same Kod:', existingOpportunity);
            // Allow updating existing opportunity instead of blocking
            console.log('Debug - Allowing update of existing opportunity');
          }
        } else {
          console.log('Debug - Skipping check: kodIndex =', kodIndex);
          console.log('Debug - selectedRowData.Kod =', selectedRowData.Kod);
        }
      }
  ...
  function processOpportunityForm(formData) {
    ...
    try {
      ...
      updateFormatTableRow(activeSheet, activeRange.getRow(), formData.firsatDurumu || 'Yeniden Aranacak', formData);
  ...
  function createOpportunityInFirsatlarim(spreadsheet, rowData, opportunityData) {
    ...
    if (kodColumnIndex > 0) {
      firsatlarimSheet.getRange(nextRow, kodColumnIndex, 1, 1).setNumberFormat('@');
        ...
        // Apply data validation to the new row with maximum force
      const headers = firsatlarimSheet.getRange(1, 1, 1, firsatlarimSheet.getLastColumn()).getValues()[0];
      
      // Fırsat Durumu validation for new row
      const firsatDurumuIndex = headers.indexOf('Fırsat Durumu') + 1;
      if (firsatDurumuIndex > 0) {
        const firsatDurumuOptions = ['Fırsat İletildi', 'Bilgi Verildi', 'Yeniden Aranacak', 'İlgilenmiyor', 'Ulaşılamadı'];
        ...
        firsatlarimSheet.getRange(nextRow, firsatDurumuIndex, 1, 1).clearDataValidations();
        
            // Apply validation (single flush for performance)
      firsatlarimSheet.getRange(nextRow, firsatDurumuIndex, 1, 1).setDataValidation(firsatRule);
      SpreadsheetApp.flush();
      
      // Simple validation for new row only
      console.log('✅ Applied validation to new row:', nextRow);
      
      console.log('Applied Fırsat Durumu validation to new row (optimized):', nextRow);
      }
    
    // Fırsat Tarihi validation for new row
    ...
    if (firsatTarihiIndex > 0) {
      ...
      firsatlarimSheet.getRange(nextRow, firsatTarihiIndex, 1, 1).setDataValidation(tarihRule);
      console.log('Applied Fırsat Tarihi validation to new row:', nextRow);
    }
    
    // Toplantı formatı validation for new row
    const toplantiFormatIndex = headers.indexOf('Toplantı formatı') + 1;
    if (toplantiFormatIndex > 0) {
      const toplantiRule = SpreadsheetApp.newDataValidation()
        .requireValueInList(CRM_CONFIG.MEETING_FORMAT_OPTIONS, true)
      ...
      firsatlarimSheet.getRange(nextRow, toplantiFormatIndex, 1, 1).setDataValidation(toplantiRule);
      console.log('Applied Toplantı formatı validation to new row:', nextRow);
    }
    
    // Apply color coding
    applyOpportunityColorCoding(firsatlarimSheet, nextRow);
    
            // Force refresh the color coding to ensure it's applied
          const currentHeaders = firsatlarimSheet.getRange(1, 1, 1, firsatlarimSheet.getLastColumn()).getValues()[0];
          const currentFirsatDurumuIndex = currentHeaders.indexOf('Fırsat Durumu');
          if (currentFirsatDurumuIndex !== -1) {
            const status = firsatlarimSheet.getRange(nextRow, currentFirsatDurumuIndex + 1).getValue();
            console.log('🎨 Applied color coding for status:', status, 'in row:', nextRow);
          }
    ...
    columns.forEach((column, index) => {
      switch (column) {
        ...
        case 'İsim Soyisim':
          ...
          break;
        case 'Fırsat Durumu':
          row[index] = opportunityData.firsatDurumu || 'Yeniden Aranacak';
          break;
        case 'Fırsat Tarihi':
          // Format date as DD.MM.YYYY
  ...
  function setFirsatlarimDataValidation(sheet) {
    ...
    if (firsatDurumuIndex > 0) {
      const firsatDurumuOptions = ['Fırsat İletildi', 'Bilgi Verildi', 'Yeniden Aranacak', 'İlgilenmiyor', 'Ulaşılamadı'];
      console.log('🔍 Fırsat Durumu options:', firsatDurumuOptions);
      ...
      // Clear existing validation first
      sheet.getRange(2, firsatDurumuIndex, validationRows, 1).clearDataValidations();
      
      // Skip clearing to avoid timeout
      console.log('⏭️ Skipping value clearing to avoid timeout');
      
      // Apply new validation (single flush for performance)
      const validationRange = sheet.getRange(2, firsatDurumuIndex, validationRows, 1);
      validationRange.setDataValidation(firsatRule);
      
      // Single flush for performance
      SpreadsheetApp.flush();
      
      console.log('✅ Applied Fırsat Durumu validation with options:', firsatDurumuOptions);
      console.log('✅ Validation range:', validationRange.getA1Notation());
      console.log('✅ Cleared all invalid values from existing rows');
    }
  ...
  function applyOpportunityColorCoding(sheet, rowNumber) {
    ...
    // Get the current status from the Fırsat Durumu column
    const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
    ...
    if (firsatDurumuIndex !== -1) {
      ...
      console.log('Current Fırsat Durumu status:', status);
      
      // Map status to color
      if (status === 'Fırsat İletildi') {
        color = CRM_CONFIG.COLOR_CODES['Fırsat İletildi'];
        console.log('🎨 Applied Fırsat İletildi color:', color);
        console.log('🔍 DEBUG: Fırsat İletildi status detected in row:', rowNumber);
        console.log('🔍 DEBUG: Color being applied:', color);
        console.log('🔍 DEBUG: CRM_CONFIG.COLOR_CODES[Fırsat İletildi]:', CRM_CONFIG.COLOR_CODES['Fırsat İletildi']);
        ...
        color = CRM_CONFIG.COLOR_CODES['Yeniden Aranacak'];
        console.log('🎨 Applied Yeniden Aranacak color:', color);
        console.log('🔍 DEBUG: Yeniden Aranacak status detected in row:', rowNumber);
        console.log('🔍 DEBUG: Color being applied:', color);
        console.log('🔍 DEBUG: CRM_CONFIG.COLOR_CODES[Yeniden Aranacak]:', CRM_CONFIG.COLOR_CODES['Yeniden Aranacak']);
    ...
    sheet.getRange(rowNumber, 1, 1, sheet.getLastColumn()).setBackground(color);
    console.log('Applied color:', color, 'to Fırsatlarım row:', rowNumber);
  }
  ...
  function collectFirsatlarimData(sheet) {
    console.log('Collecting Fırsatlarım data');
    
    // Проверка параметров
    if (!sheet) {
      console.error('❌ collectFirsatlarimData: sheet is undefined');
      return [];
    }
    ...
    try {
      ...
      console.log(`✅ Collected ${data.length} records from Fırsatlarım`);
      return data;
      
    } catch (error) {
      console.error('❌ Error collecting Fırsatlarım data:', error.message);
      return [];
    }
  }
    ...
    if (spreadsheetId === MANAGER_FILE_ID) {
      ...
      try {
        ...
        console.warn('Auto data validation failed:', error.message);
              ...
              // Automatically apply color coding to all sheets
          try {
            console.log('Auto-applying color coding to all sheets...');
            applyColorCodingToAllSheetsOnOpen();
            console.log('Color coding applied successfully');
          } catch (error) {
            console.warn('Auto color coding failed:', error.message);
          }
          
          // Automatically refresh data validation
          try {
            console.log('Auto-refreshing data validation...');
            forceRefreshValidationSilent();
            console.log('Data validation refreshed successfully');
          } catch (error) {
            console.warn('Auto validation refresh failed:', error.message);
          }
          
          // Skip Fırsatlarım validation on open to avoid timeout
          console.log('⏭️ Skipping Fırsatlarım validation on open to avoid timeout');
      
      // Create menus with error handling
      try {
        ...
        console.error('❌ Error creating menus:', error.message);
  ...
  /**
   * Applies color coding to all sheets on open
   */
  ...
  function onEdit(e) {
    ...
    try {
      ...
      if (isFormatTable(sheet)) {
        ...
        // Check if the edited cell is in the Aktivite column (usually column 12)
        const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
        const aktiviteIndex = headers.indexOf('Aktivite');
      ...
      if (sheetName === 'Fırsatlarım') {
        ...
        if (firsatDurumuIndex !== -1 && col === firsatDurumuIndex + 1 && row > 1) {
          ...
          // Special debug for Yeniden Aranacak
          if (newStatus === 'Yeniden Aranacak') {
            console.log('🔍 DEBUG: Yeniden Aranacak detected in onEdit!');
            console.log('🔍 DEBUG: Row:', row, 'Column:', col);
          ...
          console.log('✅ Color coding applied for Fırsat Durumu:', newStatus);
                    ...
                    // Force refresh validation immediately after status change
          try {
            console.log('🔄 Force refreshing validation after status change');
            
            // Emergency validation fix (optimized)
            const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
            const firsatDurumuIndex = headers.indexOf('Fırsat Durumu');
            
            if (firsatDurumuIndex !== -1) {
              // Clear validation for the specific column
              sheet.getRange(2, firsatDurumuIndex + 1, 1000, 1).clearDataValidations();
              
              // Re-apply validation (single flush for performance)
              const firsatDurumuOptions = ['Fırsat İletildi', 'Bilgi Verildi', 'Yeniden Aranacak', 'İlgilenmiyor', 'Ulaşılamadı'];
              console.log('🔍 Emergency validation options:', firsatDurumuOptions);
              ...
              sheet.getRange(2, firsatDurumuIndex + 1, 1000, 1).setDataValidation(firsatRule);
              SpreadsheetApp.flush();
              
              // Skip clearing to avoid timeout
              console.log('⏭️ Skipping value clearing to avoid timeout');
              
              // Force refresh validation for ALL rows
              const allRows = sheet.getLastRow();
              if (allRows > 1) {
                console.log('🔄 Force refreshing validation for ALL rows in onEdit');
                
                // Clear all validations first
                sheet.getRange(2, firsatDurumuIndex + 1, allRows - 1, 1).clearDataValidations();
                SpreadsheetApp.flush();
                
                // Re-apply validation to ALL rows
                sheet.getRange(2, firsatDurumuIndex + 1, allRows - 1, 1).setDataValidation(firsatRule);
                SpreadsheetApp.flush();
                
                console.log('✅ Applied validation to ALL rows in onEdit (1-', allRows, ')');
              }
              
              console.log('✅ Emergency validation fix applied (optimized)');
            }
            
            setFirsatlarimDataValidation(sheet);
            console.log('✅ Validation refreshed after status change');
          } catch (error) {
            console.warn('⚠️ Validation refresh failed after status change:', error.message);
          }
  ...
  function createAdminMenu() {
    ...
    try {
      ...
      ui.createMenu('ADMIN')
        ...
        .addItem('🧹 Temizle Yanlış Veriler', 'cleanInvalidData')
        .addSeparator()
        .addItem('🧪 Test onEdit Trigger', 'testOnEditTrigger')
        .addItem('🎨 Test Color Coding', 'testColorCoding')
        .addItem('🔍 Test Haftalık Rapor', 'testMonthlyReport')
        .addSeparator()
        .addItem('Senkronize Et', 'manualSync')
        .addToUi();
        
      console.log('✅ Admin menu created successfully');
      
    } catch (error) {
      console.error('❌ Error creating admin menu:', error.message);
      console.error('❌ Error stack:', error.stack);
    }
  }
  ...
  /**
   * Silent version of force refresh validation (no UI alerts)
   */
  function forceRefreshValidationSilent() {
    console.log('🔄 Silent refresh of data validation for all sheets');
    ...
    try {
      ...
      for (const sheet of sheets) {
        ...
        if (sheetName === 'Fırsatlarım') {
          console.log('🔄 Silent refresh validation for Fırsatlarım');
          setFirsatlarimDataValidation(sheet);
          refreshedCount++;
        } else if (isFormatTable(sheet)) {
          console.log('🔄 Silent refresh validation for Format Tablo:', sheetName);
          setDataValidation(sheet);
          refreshedCount++;
        } else if (sheetName === 'Randevularım') {
          console.log('🔄 Silent refresh validation for Randevularım');
          setRandevularimDataValidation(sheet);
          refreshedCount++;
        }
      }
      
      console.log(`✅ Silent validation refresh completed for ${refreshedCount} sheets`);
      
    } catch (error) {
      console.error('❌ Error in silent validation refresh:', error);
    }
  }
  ...
  /**
   * Cleans invalid data from all sheets
   */
  function cleanInvalidData() {
    console.log('🧹 Cleaning invalid data from all sheets');
    ...
    try {
      ...
      for (const sheet of sheets) {
        ...
        if (sheetName === 'Fırsatlarım') {
          console.log('🧹 Cleaning Fırsatlarım sheet');
          
          const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
          const firsatDurumuIndex = headers.indexOf('Fırsat Durumu');
          
          if (firsatDurumuIndex !== -1) {
            const dataRange = sheet.getRange(2, firsatDurumuIndex + 1, sheet.getLastRow() - 1, 1);
            const values = dataRange.getValues();
            
            const validOptions = ['Fırsat İletildi', 'Bilgi Verildi', 'Yeniden Aranacak', 'İlgilenmiyor', 'Ulaşılamadı'];
            
            for (let i = 0; i < values.length; i++) {
              const value = values[i][0];
              if (value && value !== '' && !validOptions.includes(value)) {
                console.log('🧹 Clearing invalid value:', value, 'in row:', i + 2);
                sheet.getRange(i + 2, firsatDurumuIndex + 1).clearContent();
                cleanedCount++;
              }
            }
            ...
            sheet.getRange(2, firsatDurumuIndex + 1, sheet.getLastRow() - 1, 1).setDataValidation(firsatRule);
            console.log('✅ Re-applied validation to Fırsatlarım');
          }
          
        } else if (isFormatTable(sheet)) {
          console.log('🧹 Cleaning Format Tablo:', sheetName);
          
          const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
          const aktiviteIndex = headers.indexOf('Aktivite');
          
          if (aktiviteIndex !== -1) {
            const dataRange = sheet.getRange(2, aktiviteIndex + 1, sheet.getLastRow() - 1, 1);
            const values = dataRange.getValues();
            
            const validOptions = ['Randevu Alındı', 'İleri Tarih Randevu', 'Yeniden Aranacak', 'Bilgi Verildi', 'Fırsat İletildi', 'İlgilenmiyor', 'Ulaşılamadı'];
            
            for (let i = 0; i < values.length; i++) {
              const value = values[i][0];
              if (value && value !== '' && !validOptions.includes(value)) {
                console.log('🧹 Clearing invalid value:', value, 'in row:', i + 2);
                sheet.getRange(i + 2, aktiviteIndex + 1).clearContent();
                cleanedCount++;
              }
            }
            ...
            sheet.getRange(2, aktiviteIndex + 1, sheet.getLastRow() - 1, 1).setDataValidation(aktiviteRule);
            console.log('✅ Re-applied validation to Format Tablo');
          }
        }
      }
      
      SpreadsheetApp.getUi().alert('✅ Очистка завершена', 
        `🧹 Очищено ${cleanedCount} неправильных значений\n\n` +
        '✅ Валидация переприменена ко всем строкам\n\n' +
        'Теперь выберите правильные значения из dropdown', 
        SpreadsheetApp.getUi().ButtonSet.OK);
      
    } catch (error) {
      console.error('❌ Error cleaning invalid data:', error);
      SpreadsheetApp.getUi().alert('❌ Ошибка очистки', 'Ошибка: ' + error.message);
    }
  }
  
  /**
   * Test function for color coding
   */
  function testColorCoding() {
    ...
    try {
      ...
      if (sheetName === 'Fırsatlarım') {
        console.log('🎨 Testing Fırsatlarım color coding');
        ...
        SpreadsheetApp.getUi().alert('✅ Test Complete', 
          `🎨 Color coding tested for row ${rowNumber}\n\n` +
          'Check console logs for details', 
          SpreadsheetApp.getUi().ButtonSet.OK);
          
      } else if (isFormatTable(sheet)) {
        console.log('🎨 Testing Format Tablo color coding');
        ...
        if (aktiviteIndex !== -1) {
          const activity = sheet.getRange(rowNumber, aktiviteIndex + 1).getValue();
          console.log('🎨 Testing activity:', activity, 'in row:', rowNumber);
          
          // Test color coding
          applyFormatTableColorCoding(sheet, rowNumber, activity);
          
          SpreadsheetApp.getUi().alert('✅ Test Complete', 
            `🎨 Color coding tested for activity "${activity}" in row ${rowNumber}\n\n` +
            'Check console logs for details', 
            SpreadsheetApp.getUi().ButtonSet.OK);
        } else {
          SpreadsheetApp.getUi().alert('❌ Error', 'Aktivite column not found', SpreadsheetApp.getUi().ButtonSet.OK);
        }
        
      } else {
        SpreadsheetApp.getUi().alert('❌ Error', 
          `This test only works on Fırsatlarım or Format Tablo sheets\n\n` +
  ...
  function applyFirsatlarimAppointmentColorCoding(sheet, rowNumber) {
    ...
    // Проверка параметров
    if (!sheet) {
      console.error('❌ applyFirsatlarimAppointmentColorCoding: sheet is undefined');
      return;
    }
    
    if (!rowNumber) {
      console.error('❌ applyFirsatlarimAppointmentColorCoding: rowNumber is undefined');
      return;
    }
    ...
    try {
      const color = CRM_CONFIG.COLOR_CODES['Randevu Alındı'];
      sheet.getRange(rowNumber, 1, 1, sheet.getLastColumn()).setBackground(color);
      console.log('✅ Applied appointment color coding to row:', rowNumber);
    } catch (error) {
      console.error('❌ Error applying appointment color coding:', error.message);
    }
  }
