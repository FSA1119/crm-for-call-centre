...
function createNewTable(parameters) {
  try {
    ...
    const response = ui.prompt('📋 Yeni Format Tablo Oluştur', 'Yeni Format Tablo için isim girin:\n\nÖrnek: t10, t11, dataset1', ui.ButtonSet.OK_CANCEL);
...
function setDataValidation(sheet) {
  ...
  // Veri olan satır sayısını bul (başlık hariç)
  const lastRow = sheet.getLastRow();
  const dataRowCount = lastRow > 1 ? lastRow - 1 : 0; // Başlık satırı hariç

  console.log(`📋 Validation uygulanıyor: ${dataRowCount} satır (toplam ${lastRow} satır)`);

  if (aktiviteIndex > 0 && dataRowCount > 0) {
    ...
    // Sadece veri olan satırlara uygula (2. satırdan başla, dataRowCount kadar)
    const validationRange = sheet.getRange(2, aktiviteIndex, dataRowCount, 1);
    validationRange.clearDataValidations();
    SpreadsheetApp.flush(); // ✅ ZORUNLU!
    validationRange.setDataValidation(aktiviteRule);
    SpreadsheetApp.flush(); // ✅ ZORUNLU!
    
    console.log(`✅ Aktivite dropdown ${dataRowCount} satıra uygulandı`);
  }

  if (toplantiFormatIndex > 0 && dataRowCount > 0) {
    ...
    // Sadece veri olan satırlara uygula (2. satırdan başla, dataRowCount kadar)
    const toplantiValidationRange = sheet.getRange(2, toplantiFormatIndex, dataRowCount, 1);
    toplantiValidationRange.clearDataValidations();
    SpreadsheetApp.flush(); // ✅ ZORUNLU!
    toplantiValidationRange.setDataValidation(toplantiRule);
    SpreadsheetApp.flush(); // ✅ ZORUNLU!
    
    console.log(`✅ Toplantı formatı dropdown ${dataRowCount} satıra uygulandı`);
  }
  
  if (dataRowCount === 0) {
    console.log('ℹ️ Veri olmadığı için validation uygulanmadı');
  }
}
...
/**
 * Test function for meeting creation
 */
 ...
 */
function onOpen() {
  console.log('🚀 onOpen() - Menüler oluşturuluyor...');
  ...
  try {
    ...
    console.log('📋 Manager File ID:', CRM_CONFIG.MANAGER_FILE_ID);
    
    // Check if this is the Manager file
    if (spreadsheetId === CRM_CONFIG.MANAGER_FILE_ID) {
      console.log('📋 Manager dosyası tespit edildi - SYNC menüsü oluşturuluyor');
      try {
        createManagerMenu();
        console.log('✅ SYNC menüsü oluşturuldu');
      } catch (error) {
        console.error('❌ SYNC menüsü oluşturulamadı:', error.message);
      }
    } else {
      console.log('📋 Temsilci dosyası tespit edildi - CRM ve ADMIN menüleri oluşturuluyor');
      
      // Automatically apply data validation to all sheets (optional, can fail silently)
      try {
        console.log('🔄 Tüm sayfalara data validation uygulanıyor...');
        refreshFormatTabloValidation({ silent: true });
        console.log('✅ Data validation başarıyla uygulandı');
      } catch (error) {
        console.warn('⚠️ Data validation uygulanamadı (devam ediliyor):', error.message);
      }
      
      // Create admin menu for all sheets
      try {
        console.log('🔄 Admin menüsü oluşturuluyor...');
        createAdminMenu();
        console.log('✅ Admin menüsü oluşturuldu');
      } catch (error) {
        console.error('❌ Admin menüsü oluşturulamadı:', error.message);
        console.error(error.stack);
      }
      
      // Create CRM menu for all sheets
      try {
        console.log('🔄 CRM menüsü oluşturuluyor...');
        const ui = SpreadsheetApp.getUi();
        ...
        try {
          ...
          if (crmMenu) {
            crmMenu.remove();
            console.log('🗑️ Eski CRM menüsü kaldırıldı');
          }
        } catch (error) {
          console.log('ℹ️ Mevcut CRM menüsü bulunamadı (normal)');
        }
        ...
        const crmMenu = ui.createMenu('CRM')
          ...
          .addSeparator()
          .addItem('📊 Data Report Oluştur', 'createDataReport');
        
        crmMenu.addToUi();
        console.log('✅ CRM menüsü oluşturuldu ve eklendi');
      } catch (error) {
        console.error('❌ CRM menüsü oluşturulamadı:', error.message);
        console.error(error.stack);
      }
    }
    
    console.log('✅ onOpen() tamamlandı - Menüler hazır!');
  } catch (error) {
    console.error('❌ onOpen() kritik hatası:', error.message);
    console.error(error.stack);
    
    // Hata durumunda bile en azından basit bir menü oluşturmayı dene
    try {
      const ui = SpreadsheetApp.getUi();
      const errorMenu = ui.createMenu('⚠️ Hata')
        .addItem('Menü oluşturulamadı - Konsolu kontrol et', 'testOnOpen');
      errorMenu.addToUi();
    } catch (e) {
      console.error('❌ Hata menüsü bile oluşturulamadı:', e.message);
    }
  }
}

/**
 * Test function to manually trigger menu creation
 * Use this if onOpen() doesn't work automatically
 */
function testOnOpen() {
  console.log('🧪 testOnOpen() - Manuel menü oluşturma testi');
  onOpen();
  SpreadsheetApp.getUi().alert('Menüler oluşturuldu! Sayfayı yenileyin (F5)');
}
...
/**
 * 🎨 Tüm Sayfalarda Renklendirme - Manager dosyası için
 */
function applyColorCodingToAllManagerSheets() {
  console.log('🎨 Tüm sayfalarda renklendirme başlatılıyor...');
  
  try {
    // Manager dosyası kontrolü
    const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
    const spreadsheetId = spreadsheet.getId();
    
    if (spreadsheetId !== CRM_CONFIG.MANAGER_FILE_ID) {
      SpreadsheetApp.getUi().alert('Bu fonksiyon sadece Manager dosyasında çalışır');
      return;
    }
    
    // Tüm sayfalarda renklendirme
    const sheets = spreadsheet.getSheets();
    ...
    for (const sheet of sheets) {
      ...
      // Sadece T sayfalarında renklendirme yap
      if (sheetName.startsWith('T ') && sheet.getLastRow() > 1) {
        try {
          console.log(`🎨 Renklendiriliyor: ${sheetName}`);
          
          // Manager sync fonksiyonunu çağır
          if (typeof applyColorCodingToManagerData === 'function') {
            const lastRow = sheet.getLastRow();
            applyColorCodingToManagerData(sheet, sheetName, 2, lastRow - 1);
            processedCount++;
            console.log(`✅ ${sheetName} renklendirildi`);
          } else {
            console.log(`⚠️ applyColorCodingToManagerData fonksiyonu bulunamadı`);
          }
        } catch (error) {
          console.error(`❌ ${sheetName} renklendirilemedi:`, error.message);
        }
      }
    }
    ...
    SpreadsheetApp.getUi().alert(
      ...
      `${processedCount} sayfa başarıyla renklendirildi.`, 
      SpreadsheetApp.getUi().ButtonSet.OK
    );
    
    console.log(`✅ Tüm sayfalarda renklendirme tamamlandı: ${processedCount} sayfa`);
    
  } catch (error) {
    console.error('❌ Tüm sayfalarda renklendirme hatası:', error);
    SpreadsheetApp.getUi().alert('Hata', 'Renklendirme sırasında hata oluştu: ' + error.message, SpreadsheetApp.getUi().ButtonSet.OK);
  }
}
...
function createAdminMenu() {
  ...
  try {
    ...
    menu.addItem('🎨 Tüm Sayfalarda Renklendirme', 'applyColorCodingToAllManagerSheets');
    menu.addItem('🧪 Test Fırsat İletildi', 'testFirsatIletildi');
    menu.addItem('🧪 Test Monthly Report', 'testMonthlyReport');
    menu.addSeparator();
    menu.addItem('🔧 Data Validation Düzelt (Dropdown/Datepicker)', 'fixDataValidationForExistingSheets');
    ...
    menu.addSeparator();
    menu.addItem('🎨 E-ticaret Kontrolü & İşaretleme', 'generateCategoryKeywordCMSReport');
...
function generateDailyReport() {
  ...
  try {
      ...
      if (durumColIndex !== -1 && aktiviteTarihiColIndex !== -1) {
        for (let i = 1; i < data.length; i++) {
          ...
          if (isToday && durum) {
            ...
            if (durum === 'İlgilenmiyor' || durum === 'Ulaşılamadı' || durum === 'Geçersiz Numara') {
              if (stats.hasOwnProperty(durum)) {
                stats[durum]++;
                console.log(`${durum} sayısı: ${stats[durum]}`);
              }
    ...
    const reportData = [
      ...
      [isFirstColumn ? '8. Geçersiz Numara' : ''],
      [''],
    ...
    if (isFirstColumn) {
      ...
      raporlarimSheet.getRange(17, 1).setValue('8. Geçersiz Numara');
      raporlarimSheet.getRange(19, 1).setValue('📈 TOPLAM İŞLEM');
    }
    ...
    const todayNumbers = [
      ...
      stats['Geçersiz Numara'] || 0, // 8. Geçersiz Numara
      '', // Boş satır
    ...
    const totalToday = totalContact + (stats['Ulaşılamadı'] || 0) + (stats['Geçersiz Numara'] || 0);
    todayNumbers[18] = totalToday; // TOPLAM İŞLEM
    ...
    raporlarimSheet.getRange(19, todayColumn).setBackground('#e3f2fd'); // TOPLAM İŞLEM - açık mavi
...
function generateMonthlyReport() {
  ...
  try {
    ...
    // Geçersiz Numara satırını ekle
    const gecersizNumaraRow = ['8. Geçersiz Numara'];
    let gecersizNumaraGrandTotal = 0;
    
    for (let colIndex = 1; colIndex < reportData[0].length; colIndex++) {
      const count = getCountForDateAndCategory(randevularimSheet, firsatlarimSheet, formatTableSheets, weekDates[colIndex - 1], '8. Geçersiz Numara');
      gecersizNumaraRow.push(count);
      if (colIndex < reportData[0].length - 1) { // Total sütunu hariç
        gecersizNumaraGrandTotal += count;
      }
    }
    
    gecersizNumaraRow.push(gecersizNumaraGrandTotal);
    reportData.push(gecersizNumaraRow);
    console.log('Geçersiz Numara satırı eklendi:', gecersizNumaraRow);
    
    // TOPLAM İŞLEM satırı ekle (Toplam Kontak + Ulaşılamadı + Geçersiz Numara)
    ...
    for (let colIndex = 1; colIndex < reportData[0].length; colIndex++) {
      ...
      // Toplam Kontak + Ulaşılamadı + Geçersiz Numara formülü
      // Kontak satırından al
      const kontakCount = reportData[reportData.length - 3][colIndex] || 0; // Kontak satırı
      const ulasilamadiCount = reportData[reportData.length - 2][colIndex] || 0; // Ulaşılamadı satırı
      const gecersizNumaraCount = reportData[reportData.length - 1][colIndex] || 0; // Geçersiz Numara satırı
      
      columnTotal = kontakCount + ulasilamadiCount + gecersizNumaraCount;
            ...
            try {
              if (isDateMatch(aktiviteTarihi, date)) {
                ...
                if (aktivite === category || 
                    ...
                    (category === '8. Geçersiz Numara' && aktivite === 'Geçersiz Numara')) {
                  count++;
...
// ========================================
// İŞ 2: DATA REPORT FONKSİYONU
// ========================================

/**
 * 📊 Data Report Oluştur - Format Tablo'dan özet rapor ve tekrar kontrolü
 * @returns {Object} - Rapor sonuçları
 */
function createDataReport() {
  const startTime = Date.now();
  console.log('📊 Data Report oluşturuluyor...');
  
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const ui = SpreadsheetApp.getUi();
    
    // 1. Format Tablo sayfasını bul
    const formatTableSheet = ss.getActiveSheet();
    if (!isFormatTable(formatTableSheet)) {
      ui.alert('Hata', 'Lütfen bir Format Tablo sayfasını açın!', ui.ButtonSet.OK);
      return { success: false, error: 'Format Tablo sayfası bulunamadı' };
    }
    
    const sheetName = formatTableSheet.getName();
    console.log(`📋 Format Tablo bulundu: ${sheetName}`);
    
    // 2. Veriyi oku (BATCH OPERATIONS - Google best practice)
    const lastRow = formatTableSheet.getLastRow();
    if (lastRow <= 1) {
      ui.alert('Bilgi', 'Format Tablo\'da veri bulunamadı.', ui.ButtonSet.OK);
      return { success: false, error: 'Veri yok' };
    }
    
    const lastCol = formatTableSheet.getLastColumn();
    const headers = formatTableSheet.getRange(1, 1, 1, lastCol).getDisplayValues()[0];
    const allData = formatTableSheet.getRange(2, 1, lastRow - 1, lastCol).getDisplayValues(); // 1 API call!
    
    console.log(`📊 ${allData.length} satır veri okundu`);
    
    // 3. Kolon indekslerini bul
    const companyIndex = headers.indexOf('Company name');
    const phoneIndex = headers.indexOf('Phone');
    const cmsNameIndex = headers.indexOf('CMS Adı');
    
    if (companyIndex === -1 || phoneIndex === -1) {
      ui.alert('Hata', 'Company name veya Phone kolonu bulunamadı!', ui.ButtonSet.OK);
      return { success: false, error: 'Gerekli kolonlar bulunamadı' };
    }
    ...
    let ideasoftCount = 0;
    let rakipCount = 0;
    let tekrarEdenCount = 0;
    
    // Temsilci kodu bul (dosya adından veya başka yerden)
    const employeeCode = getCurrentEmployeeCode() || 'Bilinmiyor';
    
    // CMS Adı kontrolü (batch işlem)
    const ideasoftRows = [];
    const rakipRows = [];
    
    for (let i = 0; i < allData.length; i++) {
      const cmsName = cmsNameIndex !== -1 ? String(allData[i][cmsNameIndex] || '').toLowerCase().trim() : '';
      
      if (cmsName.includes('ideasoft')) {
        ideasoftCount++;
        ideasoftRows.push(i + 2); // Satır numarası (2'den başlıyor)
      } else if (cmsName && cmsName !== '' && cmsName !== 'sitesi yok' && cmsName !== 'erişilemedi' && cmsName !== 'tespit edilemedi') {
        rakipCount++;
        rakipRows.push(i + 2);
      }
    }
    
    console.log(`📊 Özet: Toplam=${totalRows}, IdeaSoft=${ideasoftCount}, Rakip=${rakipCount}`);
    
    // 5. TEKRAR KONTROLÜ - Data Report History'den oku
    let historySheet = ss.getSheetByName('Data Report History');
    const existingData = new Map(); // Firma Adı + Telefon -> Tarih
    
    if (historySheet) {
      const historyLastRow = historySheet.getLastRow();
      if (historyLastRow > 1) {
        const historyHeaders = historySheet.getRange(1, 1, 1, historySheet.getLastColumn()).getDisplayValues()[0];
        const historyData = historySheet.getRange(2, 1, historyLastRow - 1, historySheet.getLastColumn()).getDisplayValues(); // 1 API call!
        
        const historyDateIndex = historyHeaders.indexOf('Tarih');
        const historyCompanyIndex = historyHeaders.indexOf('Firma Adı');
        const historyPhoneIndex = historyHeaders.indexOf('Telefon');
        
        if (historyDateIndex !== -1 && historyCompanyIndex !== -1 && historyPhoneIndex !== -1) {
          for (let i = 0; i < historyData.length; i++) {
            const company = String(historyData[i][historyCompanyIndex] || '').trim();
            const phone = String(historyData[i][historyPhoneIndex] || '').trim();
            const date = historyData[i][historyDateIndex];
            
            if (company && phone) {
              const key = `${company.toLowerCase()}|${phone.replace(/\D+/g, '')}`;
              if (!existingData.has(key) || (date && existingData.get(key) < date)) {
                existingData.set(key, date);
              }
            }
          }
        }
      ...
      // History sayfası yoksa oluştur
      historySheet = ss.insertSheet('Data Report History');
      historySheet.getRange(1, 1, 1, 3).setValues([['Tarih', 'Firma Adı', 'Telefon']])
        .setFontWeight('bold')
        .setBackground('#4A90E2');
      historySheet.setFrozenRows(1);
      console.log('✅ Data Report History sayfası oluşturuldu');
    }
    
    // 6. Tekrar eden satırları bul ve kırmızı yap
    const today = new Date();
    const tekrarEdenSatirlar = [];
    
    // Önce validation temizle + flush (renklendirme öncesi)
    if (allData.length > 0) {
      const dataRange = formatTableSheet.getRange(2, 1, allData.length, lastCol);
      dataRange.clearDataValidations();
      SpreadsheetApp.flush(); // ✅ ZORUNLU!
    }
    
    // Batch renklendirme için renk array'i hazırla
    const colors = [];
    const rowsToColor = [];
    
    for (let i = 0; i < allData.length; i++) {
      const rowNum = i + 2; // Satır numarası (2'den başlıyor)
      const company = String(allData[i][companyIndex] || '').trim();
      const phone = String(allData[i][phoneIndex] || '').trim();
      
      if (company && phone) {
        const phoneDigits = phone.replace(/\D+/g, '');
        const key = `${company.toLowerCase()}|${phoneDigits}`;
        
        if (existingData.has(key)) {
          tekrarEdenCount++;
          tekrarEdenSatirlar.push(rowNum);
          rowsToColor.push({ row: rowNum, index: i });
        }
      }
    }
    
    // Batch renklendirme (Google best practice)
    if (rowsToColor.length > 0) {
      console.log(`⚠️ ${rowsToColor.length} tekrar eden satır bulundu, kırmızı yapılıyor...`);
      
      // Her satır için renk array'i oluştur
      for (let i = 0; i < allData.length; i++) {
        const isDuplicate = rowsToColor.some(item => item.index === i);
        if (isDuplicate) {
          colors.push(Array(lastCol).fill('#FCE8E6')); // Kırmızı
        } else {
          colors.push(Array(lastCol).fill(null)); // Değişiklik yok
        }
      }
      
      // Batch renklendirme (1 API call!)
      const dataRange = formatTableSheet.getRange(2, 1, allData.length, lastCol);
      dataRange.setBackgrounds(colors);
      SpreadsheetApp.flush(); // ✅ ÖNERİLİR!
      
      console.log(`✅ ${rowsToColor.length} satır kırmızı yapıldı`);
    }
    
    // 7. Bugünkü veriyi History'ye kaydet (BATCH WRITE)
    const historyDataToWrite = [];
    for (let i = 0; i < allData.length; i++) {
      const company = String(allData[i][companyIndex] || '').trim();
      const phone = String(allData[i][phoneIndex] || '').trim();
      
      if (company && phone) {
        historyDataToWrite.push([today, company, phone]);
      }
    }
    
    if (historyDataToWrite.length > 0) {
      const historyLastRow = historySheet.getLastRow();
      const insertRow = historyLastRow + 1;
      historySheet.getRange(insertRow, 1, historyDataToWrite.length, 3).setValues(historyDataToWrite); // 1 API call!
      console.log(`✅ ${historyDataToWrite.length} satır History'ye kaydedildi`);
    }
    
    // 8. Rapor sayfası oluştur/güncelle
    let reportSheet = ss.getSheetByName('Data Report');
    if (!reportSheet) {
      reportSheet = ss.insertSheet('Data Report');
      reportSheet.getRange(1, 1, 1, 6).setValues([['Rapor Tarihi', 'Toplam', 'İdeasoft', 'Rakip', 'Temsilci', 'Tekrar Eden']])
        .setFontWeight('bold')
        .setBackground('#4A90E2');
      reportSheet.setFrozenRows(1);
      reportSheet.autoResizeColumns(1, 6);
      console.log('✅ Data Report sayfası oluşturuldu');
    }
    ...
    reportSheet.getRange(reportLastRow + 1, 1, 1, 6).setValues([reportRow]);
    reportSheet.autoResizeColumns(1, 6);
    
    // 9. Temsilci kodu sorma ve Format Tablo'ya yazma
    console.log('📋 Temsilci kodu soruluyor...');
    const codeResponse = ui.prompt(
      '👤 Temsilci Kodu Atama',
      `Format Tablo'ya temsilci kodu atamak için seçenek:\n\n` +
      `1️⃣ Tüm tablo için tek kod: Tüm satırlar için temsilci kodunu yazın\n` +
      `2️⃣ Aralık belirle: "2-50 LG001" formatında yazın (satır 2-50 için LG001)\n` +
      `3️⃣ İptal: Kod atamadan devam et\n\n` +
      `Örnek: "LG001" veya "2-50 LG001" veya "10-100 SB004"`,
      ui.ButtonSet.OK_CANCEL
    );
    
    let finalEmployeeCode = employeeCode; // Varsayılan
    
    if (codeResponse.getSelectedButton() === ui.Button.OK) {
      const codeInput = codeResponse.getResponseText().trim();
      ...
      if (codeInput) {
        ...
        if (kodIndex === -1) {
          console.warn('⚠️ Kod kolonu bulunamadı, kod atama atlandı');
        } else {
          // Aralık kontrolü: "2-50 LG001" formatı
          const rangeMatch = codeInput.match(/^(\d+)-(\d+)\s+([A-Z]{2}\s?\d{3})$/i);
          
          if (rangeMatch) {
            // Aralık belirtilmiş
            const startRow = parseInt(rangeMatch[1]);
            const endRow = parseInt(rangeMatch[2]);
            const code = rangeMatch[3].toUpperCase().replace(/\s+/g, ' ');
            
            if (startRow >= 2 && endRow <= lastRow && startRow <= endRow) {
              // Aralık için kod yaz (BATCH OPERATIONS)
              const rangeLength = endRow - startRow + 1;
              const codesToWrite = Array(rangeLength).fill([code]);
              
              // Validation temizle + flush (kod yazmadan önce)
              const kodRange = formatTableSheet.getRange(startRow, kodIndex + 1, rangeLength, 1);
              kodRange.clearDataValidations();
              SpreadsheetApp.flush(); // ✅ ZORUNLU!
              
              // Kod yaz (BATCH)
              kodRange.setValues(codesToWrite);
              kodRange.setNumberFormat('@'); // Text format
              SpreadsheetApp.flush(); // ✅ ÖNERİLİR!
              
              finalEmployeeCode = `${code} (${startRow}-${endRow})`;
              console.log(`✅ Satır ${startRow}-${endRow} için kod "${code}" yazıldı`);
            } else {
              ui.alert('Hata', `Geçersiz satır aralığı! (2-${lastRow} arası olmalı)`, ui.ButtonSet.OK);
            }
          } else {
            // Tüm tablo için tek kod
            const code = codeInput.toUpperCase().trim();
            
            // Tüm satırlar için kod yaz (BATCH OPERATIONS)
            const codesToWrite = Array(allData.length).fill([code]);
            
            // Validation temizle + flush (kod yazmadan önce)
            const kodRange = formatTableSheet.getRange(2, kodIndex + 1, allData.length, 1);
            kodRange.clearDataValidations();
            SpreadsheetApp.flush(); // ✅ ZORUNLU!
            ...
            finalEmployeeCode = code;
            console.log(`✅ Tüm tablo için kod "${code}" yazıldı`);
          }
        }
      }
    }
    
    // 10. Dataset raporunda temsilci kodunu güncelle
    if (finalEmployeeCode !== employeeCode) {
      reportSheet.getRange(reportLastRow + 1, 5).setValue(finalEmployeeCode); // 5. kolon = Temsilci
      console.log(`✅ Dataset raporunda temsilci kodu güncellendi: ${finalEmployeeCode}`);
    }
    
    // 11. Sonuçları göster
    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    const message = `📊 Data Report Oluşturuldu!\n\n` +
      `Toplam: ${totalRows} satır\n` +
      `İdeasoft: ${ideasoftCount}\n` +
      `Rakip: ${rakipCount}\n` +
      `Tekrar Eden: ${tekrarEdenCount}\n` +
      `Temsilci: ${finalEmployeeCode}\n` +
      `Süre: ${duration}s`;
    
    ui.alert('✅ Data Report', message, ui.ButtonSet.OK);
    
    console.log(`✅ Data Report tamamlandı (${duration}s)`);
    return {
      success: true,
      total: totalRows,
      ideasoft: ideasoftCount,
      rakip: rakipCount,
      tekrarEden: tekrarEdenCount,
      employeeCode: finalEmployeeCode,
      duration: parseFloat(duration)
    };
    
  } catch (error) {
    console.error('❌ Data Report hatası:', error);
    SpreadsheetApp.getUi().alert('Hata', `Data Report oluşturulamadı:\n${error.message}`, SpreadsheetApp.getUi().ButtonSet.OK);
    return { success: false, error: error.message };
  }
}

// ========================================
// İŞ 3: AKTİVİTE DROPDOWN GÜNCELLE
// ========================================

/**
 * 📋 Aktivite Dropdown Güncelle - M sütununa "Kurumsal" ekle
 * @returns {Object} - Sonuç
 */
function updateAktiviteDropdown() {
  const startTime = Date.now();
  console.log('📋 Aktivite dropdown güncelleniyor...');
  ...
  try {
    ...
    // 2. M sütununu (13. kolon = Aktivite) bul
    const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getDisplayValues()[0];
    const aktiviteIndex = headers.indexOf('Aktivite');
    
    if (aktiviteIndex === -1) {
      ui.alert('Hata', 'Aktivite kolonu bulunamadı!', ui.ButtonSet.OK);
      return { success: false, error: 'Aktivite kolonu bulunamadı' };
    }
    
    const aktiviteCol = aktiviteIndex + 1; // 1-based index
    console.log(`📋 Aktivite kolonu bulundu: ${aktiviteCol}. kolon (M)`);
    
    // 3. Yeni dropdown değerleri (9 değer - Kurumsal eklendi)
    const newValues = [
      'Randevu Alındı',
      'İleri Tarih Randevu',
      'Yeniden Aranacak',
    ...
    // 4. Eski validation'ı temizle + flush()
    const lastRow = sheet.getLastRow();
    const maxRows = sheet.getMaxRows();
    const rowsToValidate = Math.max(1000, Math.max(lastRow - 1, maxRows - 1));
    
    const validationRange = sheet.getRange(2, aktiviteCol, rowsToValidate, 1);
    validationRange.clearDataValidations();
    SpreadsheetApp.flush(); // ✅ ZORUNLU!
    
    console.log(`✅ Eski validation temizlendi (${rowsToValidate} satır)`);
    
    // 5. Yeni validation oluştur ve uygula
    const aktiviteRule = SpreadsheetApp.newDataValidation()
    ...
    console.log(`✅ Yeni validation uygulandı (9 değer: ${newValues.join(', ')})`);
    
    // 6. Sonuçları göster
    const duration = ((Date.now() - startTime) / 1000).toFixed(2);
    const message = `📋 Aktivite Dropdown Güncellendi!\n\n` +
      `Etkilenen satır: ${rowsToValidate}\n` +
      `Yeni değer sayısı: 9\n` +
      `Yeni değer: "Kurumsal" eklendi\n` +
      `Süre: ${duration}s`;
    
    ui.alert('Aktivite Dropdown', message, ui.ButtonSet.OK);
    
    console.log(`✅ Aktivite dropdown güncellendi (${duration}s)`);
    return {
      success: true,
      rowsAffected: rowsToValidate,
      valuesCount: newValues.length,
      duration: parseFloat(duration)
    };
    
  } catch (error) {
    console.error('❌ Aktivite dropdown güncelleme hatası:', error);
    SpreadsheetApp.getUi().alert('Hata', `Aktivite dropdown güncellenemedi:\n${error.message}`, SpreadsheetApp.getUi().ButtonSet.OK);
    return { success: false, error: error.message };
  }
}
