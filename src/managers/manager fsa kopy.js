...
const CRM_CONFIG = {
    ...
    COLOR_CODES: {
      ...
      'Çift Kayıt': 'rgb(255, 249, 196)'            // Light Yellow for duplicate highlight
    },
...
function parseDdMmYyyy(str) {
  ...
  function createManagerMenu() {
    try {
      ...
      // Sıralama
      menu.addItem('↕️ T Randevular: Tarihe Göre Sırala (Artan)', 'sortTRandevularStrict')
          .addSeparator();
   ...
   */
      ...
      } // if (data.length > 0) bloğunu kapatıyorum
      ...
      for (let i = 0; i < allData.length; i++) {
        ...
        if (hasAktiviteTarihi) {
          ...
          if (!valid || dt < cutoffDate) {
            ...
            if (i < 10) {
              console.log(`⏭️ 30+ gün önce (veya geçersiz tarih): "${aktiviteTarihi}" → atlandı`);
            }
            continue;
          }
        }
  ...
  function createEmployeeLogSummarySheet(managerFile, employeeCode, employeeName, logsByDate) {
    try {
      ...
      for (const dateKey of sortedDates) {
        ...
        // Saat artan (erken saat yukarıda) sırala
        dayList.sort((a, b) => toMinutesFromLog(a) - toMinutesFromLog(b));
  
        // Gün başlığı
        rows.push([`📅 ${dateKey} (${dayList.length} log)`, '', '', '', '']);
  
        // Kaynağa göre grupla
        const bySource = new Map();
        for (const item of dayList) {
          const src = String(item && item.sourceSheet || '').trim() || 'Bilinmeyen';
          if (!bySource.has(src)) bySource.set(src, []);
          bySource.get(src).push(item);
        }
  
        // Kaynakları ada göre sırala
        const sortedSources = Array.from(bySource.keys()).sort((a, b) => a.localeCompare(b, 'tr'));
  
        for (const src of sortedSources) {
          const list = bySource.get(src) || [];
          // Kaynak başlığı
          rows.push([`📊 ${src} (${list.length} log)`, '', '', '', '']);
          // Kolon başlıkları
          rows.push(columns.slice());
          // Kayıtlar
          for (const item of list) {
            const timeStr = extractTimeStr(item && item.log);
            rows.push([
              timeStr,
              String(item && item.aktivite || ''),
              String(item && item.log || ''),
              String(item && item.sourceSheet || ''),
              String(item && item.rowIndex || '')
            ]);
          }
        }
      }
      ...
      // NEW: Apply color coding to log summary for readability
      try {
        const lastRow = sheet.getLastRow();
        const lastCol = sheet.getLastColumn();
        if (lastRow > 0 && lastCol > 0) {
          ...
          for (let r = 0; r < lastRow; r++) {
            ...
            // Data rows only: color by Aktivite via centralized codes
            const activity = values[r][1];
            const color = getActivityColor(activity);
            if (color) rowBgs.fill(color);
